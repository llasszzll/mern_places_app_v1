...
const createPlace = async (req, res, next) => {
  ...
  // Push means to add new place. SESS = session
  // startTransaction / commitTransaction (mongoose module)
  try {
...
const deletePlace = async (req, res, next) => {
  ...
  // check if a placeID already exist
  if (!place) {
    const error = new HttpError(
      'Could not find a place for this ID.', 404
    );
    return next (error);
  }
...
// if it is async, need to add await